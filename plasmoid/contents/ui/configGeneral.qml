...
KCM.SimpleKCM {
    ...
    property alias cfg_donateRemindOptOut: donateOptOutCheck.checked
    ...
    // One-click local AI setup: open a terminal running `setup-local` so the
    // user sees the (multi-GB) download progress and can authenticate the
    // Ollama install. konsole on Plasma, with generic fallbacks; --hold keeps
    // the window open when it finishes.
    function runLocalSetup() {
        llmExec.exec("konsole --hold -e " + llmHelper + " setup-local"
                   + " || x-terminal-emulator -e " + llmHelper + " setup-local"
                   + " || xterm -e " + llmHelper + " setup-local");
    }
    ...
    Component.onCompleted: {
        // Migrate old ticker mode values to the new simplified set.
        if (cfg_tickerDisplayMode === "text" || cfg_tickerDisplayMode === "flagText")
            cfg_tickerDisplayMode = "short";
        else if (cfg_tickerDisplayMode === "fullName" || cfg_tickerDisplayMode === "flagFullName")
            cfg_tickerDisplayMode = "full";
    ...
    ColumnLayout {
        ...
        // Hidden backing for the deprecated tickerMatchCount config key (removed
        // from the UI because the banner only ever shows one match at a time).
        PlasmaComponents3.SpinBox { id: tickerCountSpin; visible: false; from: 1; to: 10 }
        ...
        SettingCard {
            ...
            ColumnLayout {
                ...
                Kirigami.Heading {
                    text: "Choose how the match normally looks in the system tray. This is the default view — separate from the temporary goal banner below."
                ...
                Flow {
                    ...
                    DisplayModeButton {
                        ...
                        iconText: "🇳🇴"
                        checked: cfg_trayDisplayMode === "flagFullName"
                        onClicked: cfg_trayDisplayMode = "flagFullName"
                    }
                    DisplayModeButton {
                        text: "Full name"
                        subText: "Norway 2-1 Sweden"
                        iconText: "Nor"
                        checked: cfg_trayDisplayMode === "fullName"
                        onClicked: cfg_trayDisplayMode = "fullName"
                    }
                }
        ...
        SettingCard {
            ...
            ColumnLayout {
                ...
                Kirigami.Heading {
                    text: "When a goal is scored, a temporary scrolling banner slides across the tray for a few seconds. This is independent of the normal tray display mode above."
                    level: 6; color: Kirigami.Theme.disabledTextColor; wrapMode: Text.WordWrap
                    Layout.fillWidth: true
                }

                QtObject { id: tickerModeGroup; property string checkedMode: "short" }
                ...
                Flow {
                    ...
                    DisplayModeButton {
                        text: "Teams + score"
                        subText: "⚽ GOAL! NOR 2-1 SWE"
                        iconText: "NOR"
                        checked: cfg_tickerDisplayMode === "short" || cfg_tickerDisplayMode === "text" || cfg_tickerDisplayMode === "flagText"
                        onClicked: cfg_tickerDisplayMode = "short"
                    }
                    DisplayModeButton {
                        text: "Full team names + score"
                        subText: "⚽ GOAL! Norway 2-1 Sweden"
                        iconText: "Nor"
                        checked: cfg_tickerDisplayMode === "full" || cfg_tickerDisplayMode === "fullName" || cfg_tickerDisplayMode === "flagFullName"
                        onClicked: cfg_tickerDisplayMode = "full"
                    }
                }
        ...
        // ── One-click local setup (Ollama + model + voice) ─────────────────
        PlasmaComponents3.Button {
            Kirigami.FormData.label: "Local setup:"
            text: "Set up local AI commentary (one-click)"
            icon.name: "download"
            onClicked: root.runLocalSetup()
        }
        Kirigami.Heading {
            Kirigami.FormData.label: ""
            text: "Installs everything needed for local AI commentary — Ollama, the language model and a British voice (Kokoro) — in one go. Opens a terminal to show progress; you may be asked for your password, and several GB will download."
            level: 6; color: Kirigami.Theme.disabledTextColor
            wrapMode: Text.WordWrap; Layout.fillWidth: true
            Layout.maximumWidth: Kirigami.Units.gridUnit * 22
        }

        // ── Donation reminder opt-out ──────────────────────────────────────
        PlasmaComponents3.CheckBox {
            id: donateOptOutCheck
            Kirigami.FormData.label: "Donations:"
            text: "Never show the donation reminder again"
        }
