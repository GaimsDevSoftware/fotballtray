...
Item {
    ...
    RowLayout {
        ...
        TeamBadge {
            ...
            visible: activeMatch !== null && displayMode !== "text" && displayMode !== "fullName"
            Layout.alignment: Qt.AlignVCenter
        }

        // Team name / abbreviation — its own colour + outline (style group "Lagnavn").
        OutlinedLabel {
            text: activeMatch ? teamLabel(activeMatch, true) : ""
        ...
        // Away team name / abbreviation — same "Lagnavn" style group as the home one.
        OutlinedLabel {
            text: activeMatch ? teamLabel(activeMatch, false) : ""
            level: 4; weight: Font.ExtraBold
            color: root.teamColor
            outlined: root.teamOutline
            shadow: root.teamShadow
    ...
    // Return full team name or abbreviation depending on the tray display mode.
    function teamLabel(m, isHome) {
        if (!m) return "";
        var full = isHome ? (m.homeTeam || "") : (m.awayTeam || "");
        var abbrev = isHome ? (m.homeAbbrev || m.homeTeam.substring(0, 3).toUpperCase())
                            : (m.awayAbbrev || m.awayTeam.substring(0, 3).toUpperCase());
        return (displayMode === "fullName" || displayMode === "flagFullName") ? full : abbrev;
    }
    ...
    Rectangle {
        ...
        Kirigami.Heading {
            ...
            text: {
                ...
                // New canonical values: "short" / "full". Keep old aliases for safety.
                var useFull = (mode === "full" || mode === "fullName" || mode === "flagFullName");
                var useShort = (mode === "short" || mode === "text" || mode === "flagText");
                var score = goalMatch.homeScore + "–" + goalMatch.awayScore;
                if (mode === "scoreOnly" || !useShort && !useFull) return "⚽ GOAL! " + score + " !!!";
                var h = useFull ? goalMatch.homeTeam : (goalMatch.homeAbbrev || goalMatch.homeTeam.substring(0,3).toUpperCase());
                var a = useFull ? goalMatch.awayTeam : (goalMatch.awayAbbrev || goalMatch.awayTeam.substring(0,3).toUpperCase());
                return "⚽ GOAL! " + h + " " + score + " " + a + " !!!";
            }
            color: "white"
