...
Item {
    ...
    function kickoffText(m) {
        ...
        if (s === "post" || s === "ft") {
            if (m.matchTime) {
                var pd = new Date(m.matchTime);
                if (!isNaN(pd.getTime())) return "FT · " + Qt.formatDate(pd, "d. MMM");
            }
            return "Full-time";
        }
        return m.display || "";
    ...
    // Shows time for pre AND post matches so finished games reveal when they were played.
    function kickoffSub(m) {
        if (!m || !m.matchTime) return "";
        var s = m.status || "pre";
        if (s !== "pre" && s !== "post" && s !== "ft") return "";
        var d = new Date(m.matchTime);
    ...
    // Desaturated away team colour for the right edge stripe.
    readonly property color awayStripe: {
        var raw = matchObj && matchObj.awayColor ? ("#" + matchObj.awayColor) : Kirigami.Theme.highlightColor;
        var col = Qt.darker(raw, 1.0);
        return Qt.hsla(col.hslHue, col.hslSaturation * 0.5,
                       Math.min(Math.max(col.hslLightness, 0.5), 0.72), 1.0);
    }
    ...
    // Thin team-colour edge stripes (the only accent on the row)
    Rectangle {
    ...
    ColumnLayout {
        ...
        // Venue + attendance (not shown during live)
        Kirigami.Heading {
            visible: matchObj && matchObj.venue && matchObj.venue.length > 0 && !isLive(matchObj)
            text: {
                if (!matchObj) return "";
                var v = matchObj.venue || "";
                var a = Number(matchObj.attendance || 0);
                return "📍 " + v + (a > 0 ? "  ·  Attendance: " + a.toLocaleString() : "");
            }
            ...
            ColumnLayout {
                ...
                Rectangle {
                    ...
                    RowLayout {
                        ...
                        Repeater {
                            model: ["📊 Stats", "🏟️ Tactics", "⏱️ Events", "📋 Form"]
                            Rectangle {
                ...
                StackLayout {
                    ...
                    Layout.preferredHeight: currentIndex === 0 ? statsContent.implicitHeight
                                          ...
                                          : currentIndex === 2 ? eventsContent.implicitHeight
                                          : formContent.implicitHeight
                        ...
                        Repeater {
                            ...
                            delegate: Item {
                                ...
                                Column {
                                    ...
                                    Kirigami.Heading {
                                        anchors.horizontalCenter: parent.horizontalCenter
                                        visible: modelData.hasRating && modelData.rating === detailCard.bestPitchRating() && modelData.rating >= 0
                                        text: "★"; color: "#ffd600"
                                        font.pixelSize: 14
                                    ...
                                    Rectangle {
                                        ...
                                        border.color: modelData.hasRating && modelData.rating === detailCard.bestPitchRating() && modelData.rating >= 0
                                                     ? "#ffd600" : (modelData.home ? Qt.rgba(0,0,0,0.15) : Qt.rgba(1,1,1,0.55))
                                        border.width: modelData.hasRating && modelData.rating === detailCard.bestPitchRating() && modelData.rating >= 0 ? 2.5 : 1.5
                                        Kirigami.Heading {
                                            anchors.centerIn: parent
                                    ...
                                    // Position abbreviation below jersey
                                    Kirigami.Heading {
                                        anchors.horizontalCenter: parent.horizontalCenter
                                        visible: modelData.position.length > 0
                                    ...
                                    Rectangle {
                                        ...
                                        color: modelData.hasRating && modelData.rating === detailCard.bestPitchRating() && modelData.rating >= 0
                                             ? "#ffd600" : ratingColor(modelData.rating)
                                        Kirigami.Heading {
                                            anchors.centerIn: parent
                                    ...
                                    Repeater {
                                        ...
                                        delegate: Rectangle {
                                            ...
                                            RowLayout {
                                                ...
                                                Kirigami.Heading { text: modelData.name || ""; level: 6; font.pixelSize: 10; color: Kirigami.Theme.textColor; elide: Text.ElideRight }
                                                Rectangle {
                        ...
                        Repeater {
                            ...
                            delegate: Item {
                                ...
                                RowLayout {
                                    ...
                                    // Side indicator (which team's event)
                                    Rectangle {
                                        visible: modelData.isHome !== undefined
                                        Layout.preferredWidth: 3; Layout.preferredHeight: 14; radius: 1.5
                                        Layout.alignment: Qt.AlignVCenter
                                        color: modelData.isHome ? detailCard.homeStripe : detailCard.awayStripe
                                        opacity: 0.7
                                    }
                                    ...
                                    Item {
                                        ...
                                        Text {
                                            anchors.centerIn: parent
                                            visible: evDelegate.isGoal && modelData.kind !== "owngoal"
                                            text: "⚽"
                                            font.pixelSize: 16
                                        }
                                        // Own goal — different shade
                                        Text {
                                            anchors.centerIn: parent
                                            visible: modelData.kind === "owngoal"
                                            text: "⚽"
                                            font.pixelSize: 16; opacity: 0.5
                                            color: "#c62828"
                                        }
                                        // Yellow card
                                        Rectangle {
                                        ...
                                        // Substitution — arrows (redundant, shown inline in text)
                                        Text {
                                            anchors.centerIn: parent
                                    ...
                                    Loader {
                                        Layout.fillWidth: true
                                        Layout.alignment: Qt.AlignVCenter
                                        sourceComponent: modelData.kind === "sub" ? subDisplay : textDisplay
                                    }
                                    ...
                                    Component {
                                        ...
                                        RowLayout {
                                            spacing: 2
                                            Layout.fillWidth: true
                                            readonly property var parts: modelData.text.split(" for ")
                                            Kirigami.Heading {
                                                text: "▲"; level: 6; font.pixelSize: 10; color: "#2ecc71"
                                            }
                                            Kirigami.Heading {
                                                text: subDisplay.parts.length >= 2
                                                     ? subDisplay.parts[0].trim() : modelData.text
                                                level: 6; font.pixelSize: 10; font.bold: true
                                                color: "#2ecc71"; elide: Text.ElideRight
                                            }
                                            Kirigami.Heading {
                                                visible: subDisplay.parts.length >= 2
                                                text: "▼"; level: 6; font.pixelSize: 10; color: "#e74c3c"
                                            }
                                            Kirigami.Heading {
                                                visible: subDisplay.parts.length >= 2
                                                text: subDisplay.parts[1].trim()
                                                level: 6; font.pixelSize: 10
                                                color: "#e74c3c"; elide: Text.ElideRight
                                            }
                                        }
                                    }
                                }
                    ...
                    // === FORM ===
                    ColumnLayout {
                        id: formContent; width: parent.width; spacing: 6
                        visible: currentTab === 3

                        readonly property var allMatches: root.matchData && root.matchData.matches ? root.matchData.matches : []

                        function formFor(teamName) {
                            if (!teamName || !formContent.allMatches) return [];
                            var tn = teamName.toLowerCase();
                            var results = [];
                            for (var i = 0; i < formContent.allMatches.length; i++) {
                                var m = formContent.allMatches[i];
                                if (m.status !== "post" && m.status !== "ft") continue;
                                var isHome = (m.homeTeam || "").toLowerCase() === tn;
                                var isAway = (m.awayTeam || "").toLowerCase() === tn;
                                if (!isHome && !isAway) continue;
                                var ourScore = isHome ? m.homeScore : m.awayScore;
                                var theirScore = isHome ? m.awayScore : m.homeScore;
                                var result = ourScore > theirScore ? "W" : (ourScore === theirScore ? "D" : "L");
                                results.push({
                                    result: result,
                                    opponent: isHome ? m.awayTeam : m.homeTeam,
                                    opponentAbbrev: isHome ? (m.awayAbbrev || m.awayTeam) : (m.homeAbbrev || m.homeTeam),
                                    ourScore: ourScore,
                                    theirScore: theirScore,
                                    matchTime: m.matchTime || ""
                                });
                            }
                            results.sort(function(a,b){return (a.matchTime > b.matchTime ? -1 : (a.matchTime < b.matchTime ? 1 : 0))});
                            return results.slice(0, 5);
                        }

                        function h2hFor() {
                            if (!matchObj || !formContent.allMatches) return [];
                            var home = (matchObj.homeTeam || "").toLowerCase();
                            var away = (matchObj.awayTeam || "").toLowerCase();
                            if (!home || !away) return [];
                            var meetings = [];
                            for (var i = 0; i < formContent.allMatches.length; i++) {
                                var m = formContent.allMatches[i];
                                var mh = (m.homeTeam || "").toLowerCase();
                                var ma = (m.awayTeam || "").toLowerCase();
                                if (mh === home && ma === away || mh === away && ma === home) {
                                    meetings.push({
                            ...
                            meetings.sort(function(a,b){return (a.matchTime > b.matchTime ? -1 : (a.matchTime < b.matchTime ? 1 : 0))});
                            return meetings;
                        }

                        // Home team form
                        Kirigami.Heading {
                            text: (matchObj ? matchObj.homeTeam || "Home" : "Home") + " — last 5"
                        ...
                        RowLayout {
                            spacing: 4
                            Repeater {
                                model: formContent.formFor(matchObj ? matchObj.homeTeam : "")
                                Rectangle {
                                    width: 28; height: 28; radius: 14
                                    color: modelData.result === "W" ? "#2e7d32"
                                         : modelData.result === "D" ? "#f9a825"
                                         : "#c62828"
                                    Kirigami.Heading {
                                        anchors.centerIn: parent
                                    ...
                                    ToolTip {
                                        ...
                                        text: modelData.result + " " + modelData.ourScore + "-" + modelData.theirScore + " vs " + modelData.opponent
                                        delay: 500
                                    }
                                    MouseArea {
                                        anchors.fill: parent
                        ...
                        // Away team form
                        Kirigami.Heading {
                            text: (matchObj ? matchObj.awayTeam || "Away" : "Away") + " — last 5"
                            level: 6; font.bold: true
                            color: detailCard.awayStripe
                        }
                        RowLayout {
                            spacing: 4
                            Repeater {
                                model: formContent.formFor(matchObj ? matchObj.awayTeam : "")
                                Rectangle {
                                    width: 28; height: 28; radius: 14
                                    color: modelData.result === "W" ? "#2e7d32"
                                         : modelData.result === "D" ? "#f9a825"
                        ...
                        Repeater {
                            model: formContent.h2hFor()
                            RowLayout {
                                spacing: 8
                                Kirigami.Heading {
                                    text: modelData.homeTeam + " " + modelData.homeScore + "-" + modelData.awayScore + " " + modelData.awayTeam
                                    level: 6; color: Kirigami.Theme.textColor
                                    Layout.fillWidth: true; elide: Text.ElideRight
                                }
                                Kirigami.Heading {
                                    text: modelData.league
                                    level: 7; color: Kirigami.Theme.disabledTextColor
                                }
                            }
                        }
                        Kirigami.Heading {
                            visible: formContent.h2hFor().length === 0
    ...
    function bestPitchRating() {
        var nodes = pitchNodes();
        var best = -1;
        for (var i = 0; i < nodes.length; i++) {
            if (nodes[i].hasRating && nodes[i].rating > best) best = nodes[i].rating;
        }
        return best;
    }
    function getStatsList() {
        ...
        var map = {
            ...
            "accuratePasses": {label:"Accurate passes",percentage:false},
            "tackles":        {label:"Tackles",percentage:false},
            "interceptions":  {label:"Interceptions",percentage:false},
    ...
    function cleanedEvents() {
        ...
        for (var i = 0; i < matchObj.keyEvents.length; i++) {
            ...
            out.push({ minute: String(e.clock || "").trim(), text: txt, kind: kind, isHome: e.isHome === true || e.isHome === "true" });
        }
        return out;
    }
