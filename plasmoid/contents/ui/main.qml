...
PlasmoidItem {
    ...
    // this instead of matchData to force re-evaluation even when the array identity
    // stays the same (a live goal changes homeScore but not displayMatches.length → the integer
    // model needs a fresh read to pick up the new score).
    ...
    function getDisplayMatches() {
        ...
        // Search overrides the normal follow-filter: show any match whose team names match.
        var sf = (searchFilter || "").trim().toLowerCase();
        if (sf.length > 0) {
            return all.filter(function(m) {
                return (m.homeTeam && m.homeTeam.toLowerCase().indexOf(sf) >= 0)
                    || (m.awayTeam && m.awayTeam.toLowerCase().indexOf(sf) >= 0);
            }).slice(0, maxVisibleMatches * 2);
        }
