...
- British English. Call it football.
- Reflect the flow of the match: who is dominating, has the momentum shifted, is
  one team defending deep, are there tired legs.
Hard rules:
- On a goal you MUST name the scorer AND explicitly state the current score.
- On a red card you MUST mention the score.
...
EVENT_PROMPTS = {
    "goal":     "GOAL! Erupt — call this goal and NAME THE SCORER AND THE SCORE:\n",
    "owngoal":  "OWN GOAL! Call it dramatically but with sympathy; NAME THE SCORER AND THE SCORE:\n",
    "redcard":  "RED CARD! A dramatic line; NAME THE SENT-OFF PLAYER AND THE SCORE:\n",
}
...
GENERAL_PROMPT = "Paint the run of play in a British commentator's voice — who is on top, the feel of the game, and the score. One line:\n"
...
def build_general_prompt(m):
    ...
    for key, label in (("possessionPct", "possession"), ("expectedGoals", "xG"),
...
def _clean(text: str) -> str:
    ...
    # Many reasoning models leak thinking in various formats; strip them all.
    text = re.sub(r"<think>.*?</think>", "", text, flags=re.S | re.I).strip()
    # OpenRouter/DeepSeek-style <｜end▁of▁thinking｜>/thinking blocks
    # OpenRouter/DeepSeek-style thinking/reasoning XML blocks
    text = re.sub(r"<thinking>.*?</thinking>", "", text, flags=re.S | re.I).strip()
    text = re.sub(r"<reasoning>.*?</reasoning>", "", text, flags=re.S | re.I).strip()
    # Heuristic: if there's a long chunk before the last sentence, keep only the last
    # paragraph that reads like spoken commentary (contains score, names, etc.).
    text = re.sub(r"^```.*?$|```$", "", text, flags=re.M).strip()
    for pre in ("Commentary:", "Commentator:", "Note:", "Here's", "Sure,",
                "Okay,", "OK,", "Alright,", "Let me", "I think", "I'll",
                "The commentary", "Here is", "Here are"):
        if text.lower().startswith(pre.lower()):
            text = text[len(pre):].lstrip(" :-").strip()
    text = text.strip().strip('"').strip("'").strip()
    # If the text still looks like a reasoning preamble (> 1 paragraph), keep
    # only the last line/paragraph — the actual commentary should be short.
    lines = [l for l in text.split("\n") if l.strip()]
    if len(lines) > 1:
        # Take the last meaningful-looking line (short, contains a score or name)
        last = lines[-1].strip()
        if len(last) > 10:  # avoid empty/short fragments
            return last
    return text.split("\n")[0].strip()
...
def _generate_openai_single(prompt: str, model: str) -> Optional[str]:
    """One attempt with one cloud model. Returns text or None."""
    r = requests.post(
    ...
    # 429 = rate limited; 5xx = transient. Don't raise, let caller retry.
    if r.status_code == 429:
        log.debug("Cloud model %s returned 429 (rate limited)", model)
        return None
    if 500 <= r.status_code < 600:
        log.debug("Cloud model %s returned %s", model, r.status_code)
        return None
    r.raise_for_status()
...
def generate_openai(prompt: str, model: str) -> Optional[str]:
    """Try the primary model, then fall back through other free models.

    OpenRouter's free tier is heavily rate-limited; a single model can easily
    start returning 429 during an eventful match. Cycling through alternatives
    keeps commentary flowing.
    """
    candidates = cloud_candidate_models(model)
    last_error = None
    for attempt, cand in enumerate(candidates):
        try:
            text = _generate_openai_single(prompt, cand)
            if text:
                if cand != model:
                    log.info("Cloud fallback used model: %s", cand)
                return text
        except requests.Timeout:
            log.warning("Cloud LLM timed out after %ds (%s)", OLLAMA_TIMEOUT, cand)
        except Exception as e:
            last_error = e
            log.warning("Cloud LLM generate failed for %s: %s", cand, e)
        # Tiny backoff between fallback attempts to be polite to the provider.
        if attempt < len(candidates) - 1:
            time.sleep(0.5 * (attempt + 1))
    if last_error:
        log.warning("Cloud LLM exhausted candidates (%d tried)", len(candidates))
    return None


def list_cloud_model_ids() -> list:
    """Fetch available model ids from the cloud provider."""
    if not LLM_API_KEY:
        return []
    try:
        r = requests.get(
...
def cloud_ready() -> bool:
    if not LLM_API_KEY:
        ...
        return False
    return bool(list_cloud_model_ids())


def _preferred_cloud_model(pool: list, preferred: str) -> Optional[str]:
    """Pick the best model from pool matching preferred or a capability list."""
...
def cloud_pick_model() -> Optional[str]:
    ...
    ids = list_cloud_model_ids()
    if not ids:
        return None
    free = [i for i in ids if i.endswith(":free")]
    pool = free or ids
    return _preferred_cloud_model(pool, LLM_MODEL)


def cloud_candidate_models(primary: Optional[str]) -> list:
    """Return an ordered list of models to try when one is rate-limited.

    The primary model is first, then other preferred free models, then any
    remaining free models. This makes OpenRouter's strict free-tier limits
    survivable during busy matches.
    """
    ids = list_cloud_model_ids()
    if not ids:
    ...
    if primary:
        out.append(primary)
    # Add preferred models not already in the list.
    for pref in ("deepseek/deepseek-chat", "meta-llama/llama-3.3-70b",
                 "meta-llama/llama-3.1-70b", "qwen/qwen-2.5-72b",
                 "google/gemini", "mistralai/mistral"):
        for i in pool:
            if pref in i and i not in out:
                out.append(i)
    # Add any remaining pool models.
    for i in pool:
        if i not in out:
            out.append(i)
    return out
...
def event_player_known(e: dict, kind: str) -> bool:
    """Return False if the event still has a placeholder player name.

    Fetchers sometimes report a goal with scorer '<TBD>' before the player is
    identified. Commentating twice for the same goal wastes API calls and can
    trigger rate limits, so we skip the placeholder pass.
    """
    if kind not in ("goal", "owngoal", "redcard"):
        return True
    name = event_player(e).strip().lower()
    return name and name not in ("<tbd>", "tbd", "?", "unknown", "n/a", "-")


def event_key(match_id: str, e: dict, kind: str) -> str:
    # Normalize clock: "22" and "22'" should be the same key.
    clock = str(e.get("clock", "")).strip().rstrip("'").strip()
    player = (event_player(e) or "").strip().lower()
    return f"{match_id}|{kind}|{clock}|{player}"
...
def build_prompt(kind: str, e: dict, match: dict) -> str:
    ...
    for key, label in (("possessionPct", "possession"), ("expectedGoals", "xG"),
                       ("totalShots", "shots"), ("shotsOnTarget", "shots on target"),
                       ("corners", "corners"), ("yellowCards", "yellow cards")):
        h, a = pair(key)
        if h is not None:
            details[label] = f"{h}-{a}"
...
def process(model: str, seen: dict) -> int:
    ...
    # Prune seen entries for matches no longer in the current data
    current_ids = {str(m.get("id", "")) for m in matches}
    stale = [mid for mid in seen if mid not in current_ids]
    for mid in stale:
        del seen[mid]
        _last_general.pop(mid, None)
    ...
    for m in live:
        ...
        hv = [(e, k) for (e, k) in hv if k in HIGH_VALUE and event_player_known(e, k)]
