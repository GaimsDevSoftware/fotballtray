...
try:
    ...
    pull-model)
        ...
        ;;
    setup-local)
        # One-click local AI commentary: Ollama runtime + language model +
        # British voice (Kokoro). Meant to run in a terminal so the user sees
        # the (multi-GB) download progress and can authenticate the Ollama
        # install. Fully idempotent — safe to re-run.
        M="${2:-$(current_model)}"; [ -z "$M" ] && M="$DEFAULT_MODEL"
        KDIR="$HOME/.local/share/kokoro"
        KBASE="https://github.com/thewh1teagle/kokoro-onnx/releases/download/model-files-v1.0"
        echo "════ FootballTray · local AI commentary setup ════"; echo
        echo "[1/3] Ollama (the language-model runtime)"
        if have_ollama; then
            echo "      already installed."
        else
            echo "      installing — you may be prompted for your password…"
            curl -fsSL https://ollama.com/install.sh | sh || { echo "FAILED: Ollama install error."; exit 1; }
        fi
        ensure_serving || { echo "FAILED: Ollama is not running."; exit 1; }
        echo
        echo "[2/3] Commentary model: $M  (a multi-GB download — please wait)"
        if model_installed "$M"; then
            echo "      already present."
        else
            ollama pull "$M" || { echo "FAILED: could not pull '$M'."; exit 1; }
        fi
        mkdir -p "$DROPIN_DIR"
        printf '[Service]\nEnvironment=OLLAMA_MODEL=%s\n' "$M" > "$DROPIN_DIR/model.conf"
        systemctl --user daemon-reload 2>/dev/null || true
        echo
        echo "[3/3] British voice (Kokoro text-to-speech)"
        mkdir -p "$KDIR"
        if python3 -c "import kokoro_onnx" 2>/dev/null; then
            echo "      kokoro-onnx already installed."
        else
            echo "      installing kokoro-onnx (Python)…"
            python3 -m pip install --user --quiet kokoro-onnx soundfile \
              || pip install --user kokoro-onnx soundfile \
              || echo "      WARNING: pip install failed — will fall back to piper if present."
        fi
        if [ -f "$KDIR/kokoro-v1.0.onnx" ]; then echo "      voice model present."; else
            echo "      downloading voice model (~310 MB)…"
            curl -fL --progress-bar -o "$KDIR/kokoro-v1.0.onnx" "$KBASE/kokoro-v1.0.onnx" \
              || echo "      WARNING: voice-model download failed."
        fi
        if [ -f "$KDIR/voices-v1.0.bin" ]; then echo "      voices present."; else
            echo "      downloading voices…"
            curl -fL --progress-bar -o "$KDIR/voices-v1.0.bin" "$KBASE/voices-v1.0.bin" \
              || echo "      WARNING: voices download failed."
        fi
        echo
        echo "════ Done. Turn on 'Live commentary (AI)' in FootballTray settings. ════"
        ;;
    set-model)
        M="${2:?model name required}"
